...
export class ReportNewPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public zone: NgZone,
    ...
    private myWardDataProvider: MyWardDataProvider, private authHandler: AuthHandlerProvider, private toastCtrl: ToastController) {
  ...
  createMap() {
    ...
    let prevLoc = new LatLng(13.071521, 80.241516);
  ...
  autoFillAddress() {
    ...
    this.nativeGeocoder.reverseGeocode(lat , lng).then((result: NativeGeocoderReverseResult[]) => {
      ...
      if (address.subLocality) {
        ...
        this.tagnames.push(address.subLocality);
      }
      if (address.locality) {
        str += address.locality +  ", ";
        this.tagnames.push(address.locality);
      }
      if (address.subAdministrativeArea) {
        str += address.subAdministrativeArea + ", ";
        this.tagnames.push(address.subAdministrativeArea);
      }
      if (address.administrativeArea) {
        str += address.administrativeArea + ", ";
        this.tagnames.push(address.administrativeArea);
      }
